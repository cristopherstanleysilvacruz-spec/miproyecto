      ...
      <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="text-center sm:text-left">
          <span className="line-through text-gray-400 mr-2 text-sm md:text-base">
            S/ {promo.precioAnterior}
          </span>
          <span className="text-2xl md:text-3xl font-bold">
            S/ {promo.precioActual}
          </span>
        </div>
...
Quisiera confirmar mi pedido.`;
    ...
    const whatsappUrl = `https://api.whatsapp.com/send/?phone=51914068562&text=${encodeURIComponent(
      mensaje
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      <main className="pt-10 pb-16 px-4 md:px-8">
        <h1 className="text-4xl md:text-5xl font-bold text-center mb-3 text-black">
          Promociones <span className="text-yellow-500">Especiales</span>
        </h1>

        <p className="text-center text-gray-600 text-sm md:text-base mb-8">
          Solo una promoción por cliente
        </p>
        ...
        {promociones.map((promo) => (
          <OfertaCard
            ...
            tiempoRestante={
              promoElegida?._id === promo._id ? tiempoRestante : null
            }
          />
        ))}
      </main>
